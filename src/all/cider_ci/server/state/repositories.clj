(ns cider-ci.server.state.repositories
  (:require
    [cider-ci.server.repository.state.db]
...
(defn update-repos-in-server-db []
  (swap! cider-ci.server.state.db/db
         ...
         (get @cider-ci.server.repository.state.db/db :repositories {})))

(defn watch []
  (add-watch
    cider-ci.server.repository.state.db/db
    :update-repositories-in-server-db
    (fn [_key repo-db-ref _old _new]
      (if (not= (-> @cider-ci.server.repository.state.db/db :repositories)
