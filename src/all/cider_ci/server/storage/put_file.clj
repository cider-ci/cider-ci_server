...
(ns cider-ci.server.storage.put-file
  (:require
...
(defn executor-may-upload-trial-attachments? [request]
  (or (-> request :authenticated-entity :upload_trial_attachments)
...
(defn executor-may-upload-tree-attachments? [request]
  (or (-> request :authenticated-entity :upload_tree_attachments)
            ...
            (catcher/snatch {}
              (jdbc/insert! (get-ds) :job_issues
                {:job_id job-id
                 ...
                 :type "warning"
                 :title "Attachment Upload Forbidden"
                 :description (str "The executor " (-> request :authenticated-entity :name)
                                   " is not allowed to upload attachments but it tried to.")})))
          false)))
