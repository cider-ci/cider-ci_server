...
(defn push-to-client [db user-client-id]
  (let [user-id (-> user-client-id (clojure.string/split #"_") first)
    ...
    (chsk-send! user-client-id [(keyword "cider-ci.server.repository" "db")
                           @push-data])))
...
(def routes
  (-> (cpj/routes
        (cpj/GET  "/server/ws/chsk" req (ring-ajax-get-or-ws-handshake req))
        (cpj/POST "/server/ws/chsk" req (ring-ajax-post req)))
