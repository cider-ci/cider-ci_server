...
(ns cider-ci.server.resources.commits.back
...
(defn filter-by-project-name [query {{project-name :project-name
  ...
  (if (presence project-name)
    (-> query
        (sql/merge-join :branches_commits [:= :branches_commits.commit_id :commits.id])
        (sql/merge-join :branches [:= :branches.id :branches_commits.branch_id])
        (sql/merge-join :projects [:= :projects.id :branches.project_id])
        ((fn [query]
           (if as-regex
             (sql/merge-where query ["~*" :projects.name project-name])
             (sql/merge-where query [:= :projects.name project-name])))))
    query))
...
(defn filter-by-my-commits
  [query {{my-commits :my-commits} :query-params
          {id :id} :authenticated-entity}]
  (if (-> my-commits presence boolean)
    (sql/merge-where
      query [:or
...
(defn add-branches-to-remote-query [commit remote]
  (-> (sql/select [:branches.name :name]
                  [:branches.id :key]
                  (sql/raw (str "( current_commits.depth - " (:depth commit)
                                " ) AS distance_to_head ")))
      (sql/from :branches)
      (sql/merge-where [:= :branches.project_id (:id remote)])
...
(defn add-branches-to-remote [commit remote]
  (assoc
    remote
    :branches
    (->> (add-branches-to-remote-query commit remote)
         (jdbc/query (rdbms/get-ds)))))
...
(defn add-projects [commit]
  (assoc commit :projects
         (->> (-> (sql/select :projects.name
                  ...
                  (sql/merge-join :projects
                                  [:= :branches.project_id :projects.id])
                  (sql/order-by [:projects.name :asc] [:projects.id :asc])
                  sql/format)
              (jdbc/query (rdbms/get-ds))
...
(defn project-and-branchnames [request]
  {:body
   (->> (-> (sql/select [:projects.name :project]
                        [:projects.id :key]
                        (sql/raw "array_agg(branches.name) branches "))
            (sql/from :projects)
            (sql/merge-join :branches [:= :projects.id :branches.project_id])
            (sql/group :projects.name)
            sql/format)
        (jdbc/query (rdbms/get-ds))
...
(def routes
  (-> (cpj/routes
        (cpj/GET  (path :commits) _ #'commits)
