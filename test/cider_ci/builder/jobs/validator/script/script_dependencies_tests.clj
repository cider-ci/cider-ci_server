(ns cider-ci.builder.jobs.validator.script.script-dependencies-tests
  (:require
...
(deftest test-validate!
  ...
  (testing "spec with valid script dependencies"
    (let [job-spec
          (-> "
              key: job-key
              name: job-name
  ...
  (testing "spec with in-valid script dependency without key ref"
    (let [job-spec
          (-> "
              key: job-key
              name: job-name
  ...
  (testing "spec with in-valid script dependency with illegal state"
    (let [job-spec
          (-> "
              key: job-key
              name: job-name
